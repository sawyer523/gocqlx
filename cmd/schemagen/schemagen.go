...
var (
	...
	flagKeyspace = cmd.String("keyspace", "zerosecond", "keyspace to inspect")
	flagPkgname  = cmd.String(
		"pkgname",
		"models",
		"the name you wish to assign to your generated package",
	)
	...
	flagIgnoreNames = cmd.String(
		"ignore-names",
		"",
		"a comma-separated list of table, view or index names to ignore",
	)
	...
	flagConnectionTimeout = cmd.Duration(
		"connection-timeout",
		defaultConnectionTimeout,
		"connection timeout ( in seconds )",
	)
	flagSSLEnableHostVerification = cmd.Bool(
		"ssl-enable-host-verification",
		false,
		"don't check server ssl certificate",
	)
...
func schemagen() error {
	...
	return os.WriteFile(outputPath, b, os.ModePerm)
}
...
func renderTemplate(md *gocql.KeyspaceMetadata) ([]byte, error) {
	t, err := template.
		...
		Funcs(template.FuncMap{"hasUpper": hasUpper}).
		Parse(keyspaceTmpl)
	...
	for _, t := range md.Tables {
		for _, c := range t.Columns {
			if (c.Validator == "timestamp" || c.Validator == "date" || c.Validator == "duration" || c.Validator == "time") && !existsInSlice(
				imports,
				"time",
			) {

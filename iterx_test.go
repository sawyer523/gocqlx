...
import (
	...
	"github.com/sawyer523/gocqlx/gocqlxtest"
	"github.com/sawyer523/gocqlx/qb"
)
...
func TestIterxUDT(t *testing.T) {
	...
	if err := session.ExecStmt(
		`CREATE TABLE gocqlx_test.udt_table (
	...
	for _, tc := range tcases {
		t.Run(
			tc.name, func(t *testing.T) {
				...
				t.Run(
					"insert-bind", func(t *testing.T) {
						...
						if err := session.Query(
							`SELECT testudt FROM udt_table where testuuid = ?`,
							nil,
						).Bind(testuuid).Get(&v); err != nil {
							t.Fatal(err.Error())
						}
				...
				t.Run(
					"insert-bind-struct", func(t *testing.T) {
						b := makeStruct(testuuid, tc.insert)
						if err := session.Query(
							insertStmt,
							[]string{"test_uuid", "test_udt"},
						).BindStruct(b).ExecRelease(); err != nil {
							t.Fatal(err.Error())
						}
				...
				t.Run(
					"insert-bind-struct-map", func(t *testing.T) {
						t.Run(
				...
				t.Run(
					"insert-bind-map", func(t *testing.T) {
						if err := session.Query(insertStmt, []string{"test_uuid", "test_udt"}).
							BindMap(
...
func TestIterxStruct(t *testing.T) {
	...
	if err := session.ExecStmt(
		`CREATE TABLE gocqlx_test.struct_table (
			testuuid       timeuuid PRIMARY KEY,
...
func TestIterxStructOnly(t *testing.T) {
	...
	if err := session.Query(
		`INSERT INTO struct_only_table (first_name, last_name) values (?, ?)`,
		nil,
	).Bind(m.FirstName, m.LastName).Exec(); err != nil {
		t.Fatal("insert:", err)
	}
...
func TestIterxStructOnlyUDT(t *testing.T) {
	...
	if err := session.Query(
		`INSERT INTO struct_only_udt_table (first_name, last_name) values (?, ?)`,
		nil,
	).Bind(m.FirstName, m.LastName).Exec(); err != nil {
		t.Fatal("insert:", err)
	}
...
func TestIterxStrict(t *testing.T) {
	...
	if err := session.Query(`INSERT INTO strict_table (testtext, testtextunbound) values (?, ?)`, nil).Bind(
		"test",
		"test",
	).Exec(); err != nil {
		t.Fatal("insert:", err)
	...
	t.Run(
		"select strict", func(t *testing.T) {
			var v []StrictTable
	...
	t.Run(
		"select default", func(t *testing.T) {
			var v []StrictTable
...
func TestIterxNotFound(t *testing.T) {
	...
	t.Run(
		"get cql error", func(t *testing.T) {
			var v NotFoundTable
	...
	t.Run(
		"select cql error", func(t *testing.T) {
			var v []NotFoundTable

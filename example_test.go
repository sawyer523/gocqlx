...
import (
	...
	"github.com/sawyer523/gocqlx/table"
)
...
func basicCreateAndPopulateKeyspace(t *testing.T, session gocqlx.Session, keyspace string) {
	...
	err = session.ExecStmt(
		fmt.Sprintf(
			`CREATE TABLE IF NOT EXISTS %s.playlists (
		id uuid,
		title text,
		album text, 
		artist text,
...
func createAndPopulateKeyspaceAllTypes(t *testing.T, session gocqlx.Session) {
	...
	err = session.ExecStmt(
		`CREATE TABLE IF NOT EXISTS examples.check_types (
		asci_i ascii,
		big_int bigint,
		blo_b blob,
		boolea_n boolean,
	...
	insertCheckTypes := qb.Insert("examples.check_types").
		Columns(
			"asci_i", "big_int", "blo_b", "boolea_n", "dat_e", "decima_l", "doubl_e", "duratio_n", "floa_t",
			"ine_t", "in_t", "small_int", "tex_t", "tim_e", "timestam_p", "timeuui_d", "tiny_int", "id", "var_char",
			"var_int", "map_int_text", "list_int", "set_int",
		).Query(session)
...
func pagingForwardPaging(t *testing.T, session gocqlx.Session) {
	...
	err = session.ExecStmt(
		`CREATE TABLE IF NOT EXISTS examples.paging_forward_paging(
		user_id int,
		user_name text,
		added timestamp,
		video_id int,
...
func pagingEfficientFullTableScan(t *testing.T, session gocqlx.Session) {
	...
	err = session.ExecStmt(
		`CREATE TABLE IF NOT EXISTS examples.paging_efficient_full_table_scan(
		user_id int,
		user_name text,
		added timestamp,
		video_id int,
...
func unsetEmptyValues(t *testing.T, session gocqlx.Session) {
	...
	err = session.ExecStmt(
		`CREATE TABLE IF NOT EXISTS examples.operations (
		id text PRIMARY KEY,
		client_id text,
		type text,
		payment_id text,

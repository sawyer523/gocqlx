...
func TestUpdateBuilder(t *testing.T) {
	...
	table := []struct {
		...
		{
			B: Update("cycling.cyclist_name").Set("id", "user_uuid", "firstname", "\"LastName\"").Where(w),
			S: "UPDATE cycling.cyclist_name SET id=?,user_uuid=?,firstname=?,\"LastName\"=? WHERE id=? ",
			N: []string{"id", "user_uuid", "firstname", "last_name", "expr"},
		},
		// Change table name
		{
		...
		// Add SET SetFunc
		{
			B: Update("cycling.cyclist_name").SetFunc(
				"user_uuid",
				Fn("someFunc", "param_0", "param_1"),
			).Where(w).Set("stars"),

...
func (b *SelectBuilder) Columns(columns ...string) *SelectBuilder {
	...
	for _, c := range columns {
		if hasUpper(c) {
			b.columns = append(b.columns, As(quote(c), camelToSnakeASCII(c)))
		} else {
			b.columns = append(b.columns, quote(c))
		}
	}
	return b
}

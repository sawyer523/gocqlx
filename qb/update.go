	...
	for _, c := range columns {
		b.assignments = append(
			b.assignments, assignment{
				column: quote(c),
				value:  param(camelToSnakeASCII(c)),
			},
		)
	}
...
func (b *UpdateBuilder) SetNamed(column, name string) *UpdateBuilder {
	b.assignments = append(
		b.assignments, assignment{column: quote(column), value: param(name)},
	)
	return b
}
...
func (b *UpdateBuilder) SetLit(column, literal string) *UpdateBuilder {
	b.assignments = append(
		b.assignments, assignment{column: quote(column), value: lit(literal)},
	)
	return b
}
...
func (b *UpdateBuilder) SetFunc(column string, fn *Func) *UpdateBuilder {
	b.assignments = append(b.assignments, assignment{column: quote(column), value: fn})
	return b
}
...
func (b *UpdateBuilder) SetTuple(column string, count int) *UpdateBuilder {
	b.assignments = append(
		b.assignments, assignment{
			column: quote(column),
			value: tupleParam{
				param: param(camelToSnakeASCII(column)),
				count: count,
			},
		},
	)
...
func (b *UpdateBuilder) Add(column string) *UpdateBuilder {
	return b.addValue(column, param(camelToSnakeASCII(column)))
}
...
func (b *UpdateBuilder) Remove(column string) *UpdateBuilder {
	return b.removeValue(column, param(camelToSnakeASCII(column)))
}

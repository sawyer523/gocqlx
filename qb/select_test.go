...
func TestSelectBuilder(t *testing.T) {
	...
	table := []struct {
		...
		// Add test for select columns as JSON
		{
			B: Select("cycling.cyclist_name").Columns("id", "user_uuid", "firstname", "deviceId").Json(),
			S: "SELECT JSON id,user_uuid,firstname,\"deviceId\" AS device_id FROM cycling.cyclist_name ",
		},
		// Add a SELECT AS column as JSON 2
		{
